...
class Designs:
    ...
    class Line:
        ...
        def _calculate_parameters(self):
            x1 = self.p1.x ; y1 = self.p1.y
            x2 = self.p2.x ; y2 = self.p2.y
        ...
        def draw(self):
            ...
            # to draw the edge
            plt.plot((self.p1.x, self.p2.x), (self.p1.y, self.p2.y), c="blue", zorder=99)
    ...
    class Kitchen_Sync:
        ...
        def _calculate_parameters(self):
            ...
            if self.points[0].distance(self.points[1]) < self.points[1].distance(self.points[2]):
                self.width = side_A1.mid_point.distance(side_A2.mid_point)/2 * Sync_SHRINK_FACTOR
                self.height = side_B1.mid_point.distance(side_B2.mid_point)/2 * Sync_SHRINK_FACTOR
            else:
                self.height = side_A1.mid_point.distance(side_A2.mid_point)/2 * Sync_SHRINK_FACTOR
                self.width = side_B1.mid_point.distance(side_B2.mid_point)/2 * Sync_SHRINK_FACTOR

            L1, L2 = (side_A1, side_A2) if side_A1.length+side_A2.length > side_B1.length+side_B2.length else (side_B1, side_B2)
            self.angle = min(L1.angle , L2.angle)

            # x_center, y_center = Designs._get_intersection((points[0],points[2]), (points[1],points[3]))
            x_center, y_center = Designs._get_intersection((side_A1.mid_point, side_A2.mid_point),
        ...
        def show(self):
            ...
            for i, point in enumerate(self.points):
                ...
                plt.plot((point.x, next_point.x), (point.y, next_point.y), c="blue", zorder=99)
    ...
    class Kitchen_Gas:
        ...
        def _calculate_parameters(self):
            ...
            self.radius = min(min(side_A1.length,side_A2.length), min(side_B1.length,side_B2.length))/4 * GAS_SHRINK_FACTOR

            mid_lines_1 = [(side_A1_i.mid_point,side_A2_ii.mid_point), (side_A1_ii.mid_point,side_A2_i.mid_point),
                        (side_A1_i.mid_point,side_A2_ii.mid_point), (side_A1_ii.mid_point,side_A2_i.mid_point)]
            mid_lines_2 = [(side_B1_i.mid_point,side_B2_ii.mid_point), (side_B1_i.mid_point,side_B2_ii.mid_point),
                        (side_B1_ii.mid_point,side_B2_i.mid_point), (side_B1_ii.mid_point,side_B2_i.mid_point)]

            for i in range(self.num_of_heads):
                x_center, y_center = Designs._get_intersection(mid_lines_1[i], mid_lines_2[i])
                center = Point(x_center, y_center)
                self.centers.append(center)

                circle = Circle((center.x, center.y), self.radius, fill=False, linewidth=LINE_WIDTH, color="blue")
    ...
    class Circle:
        def __init__(self, center, radius, color=0):
            self.center = center
            self.radius = radius
            self.color = color
            return
    ...
    class Electric:
        ...
        def calculate_points(self, measures_str):
            # measures_str example "60x120"
            requested_H = float(measures_str.split("x")[0])
            requested_W = float(measures_str.split("x")[1])
            center_x, center_y = Designs._get_intersection((self.points[0],self.points[2]), (self.points[1],self.points[3]))
            x1 = center_x - (requested_W/2)
            x2 = center_x + (requested_W/2)
            y1 = center_y - (requested_H/2)
            y2 = center_y + (requested_H/2)
            self.updated_points = [Point(x1,y1), Point(x1,y2), Point(x2,y2), Point(x2,y1)]
            return
